...
切分单位 = 字段语义边界，不按定长。每菜谱 → 原料 chunk + 步骤 chunk。

```
ingredients chunk  ← 全部原料合并 1 块
steps chunk        ← 全部步骤合并；超阈值按步骤边界再切
```

#### 依据 1：为何按字段切
原料与步骤检索语义不同：
- 原料 = 名词关键词（"farine"、"œuf"）→ 偏 **BM25**
- 步骤 = 动作语义（"怎么做 X"）→ 偏 **向量**

分开切 → 混合检索各取所长；`metadata.section_type` 让下游分别加权。

#### 依据 2：步骤二次切分标准 = `max_chars=800` + 步骤边界
`_split_steps()` 贪心打包，两条硬标准：

| 标准 | 原因 |
|------|------|
| **阈值 800 字符** | 控制单 chunk 不超 embedding 上下文窗口，避免编码截断 |
| **只在步骤边界切** | 单个步骤是最小语义单元，绝不切断；切半会破坏向量语义 |

```python
for s in steps:
    if cur and size + len(s) > max_chars:   # 累计超阈值
        flush(cur); cur = []                 # 切新块 (步骤边界)
    cur.append(s)
```
多块时 id `{slug}::steps::{i}` + `metadata.chunk_index` 保顺序、可拼回。

#### 依据 3：为何不用定长滑窗
| 方案 | 问题 |
|------|------|
| 定长 N 字符 + overlap | 切断句子/步骤中间，语义碎，需 overlap 补偿 → 冗余 |
| **本方案（字段 + 步骤边界）** | 天然语义单元，无需 overlap，原料/步骤分流 |

菜谱本身结构化（原料表 + 步骤序列），边界天然存在 → 按结构切优于盲目定长。

#### 阈值 800 怎么定
- 法语菜谱多数步骤段 < 800 字符 → 多数菜谱步骤 1 块（不碎）
- 仅超长菜谱（多步 / menu 类）触发切分
- 可调：`run(max_chars=...)` 或 `--max-chars`，换 embedding 模型（窗口不同）时调
